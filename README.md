...
An experimental, local-only macOS utility with a compact native control window. It transfers selected browser data between Brave, Chrome, Edge, Arc, Vivaldi, Opera, and Perplexity Comet, or into ChatGPT Codex's built-in browser. ChatGPT Codex is import-only.
...
npm run check
npm pack
npx --yes ./brave-codex-cookie-sync-0.12.0.tgz install-app
```
...
```bash
npx --yes ./brave-codex-cookie-sync-0.12.0.tgz preferences --source brave --target codex --cookies on --history off --menu-bar off
npx --yes ./brave-codex-cookie-sync-0.12.0.tgz sync --timeout 300
npx --yes ./brave-codex-cookie-sync-0.12.0.tgz setup --hour 9 --minute 0
npx --yes ./brave-codex-cookie-sync-0.12.0.tgz enable-login-sync
```
