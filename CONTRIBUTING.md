...
8. With the source browser (for example Brave) **still open** and **Grok Bot** selected, click **Create transfer file**. It must succeed: `logs/app.log` shows a `snapshot: copied Cookies…` line before the CLI launch and the CLI output reads `… from brave via the app's snapshot`. Grok Bot never requires quitting the source browser; the app copies the cookie database itself with its own Full Disk Access and hands the copy to the CLI through `BCB_SOURCE_SNAPSHOT_DIR`. Confirm `~/Library/Application Support/BraveCodexCookieSync/snapshots/` is empty again once the CLI exits.
9. Remove Browser Cookie Bridge from **System Settings › Privacy & Security › Full Disk Access** and relaunch the app. With **Grok Bot** selected the main window must show **Grant Full Disk Access to read Brave cookies** in the warning state (Create is blocked) and the button must read **Open Full Disk Access settings** and open that pane. Trigger Create from the menu bar anyway: the alert must mention Full Disk Access — never only "close the browser" — and offer **Open Full Disk Access settings**. Re-add the app, relaunch, and confirm the status returns to **Ready to create a Grok Bot transfer file**.
