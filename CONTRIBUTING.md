...
9. Remove Browser Cookie Bridge from **System Settings › Privacy & Security › Full Disk Access** and relaunch the app. With **Grok Bot** selected the main window must show **Grant Full Disk Access to Browser Cookie Bridge** in the warning state and the button must read **Grant Full Disk Access…** and open that settings pane. Trigger Create from the menu bar anyway: the alert must mention Full Disk Access — never only "close the browser" — and offer **Open Full Disk Access settings**. Re-add the app, relaunch, and confirm the status returns to **Ready to create a Grok Bot transfer file**.
